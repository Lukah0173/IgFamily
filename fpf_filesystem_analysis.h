	...
	void fout_file_genefamily_summary(fpf_filesystem::filesystem_type par_s_filesystem) {
		...
		if (IgFamily::MAP_FOUT_BY_SCORE == 0) {
			if (IgFamily::MAP_FOUT_BY_DISTINCT == 1) {
				...
				//con_map_main_v_c_analysis = par_s_filesystem.v_c_multinomial_catagory_distinct;
			}
		}
		...
		for (std::vector<fpf_data::multinomial_catagory_data_type*>::iterator itr_v_c_analysis_distinct = con_map_main_v_c_analysis.begin(); itr_v_c_analysis_distinct != con_map_main_v_c_analysis.end(); ++itr_v_c_analysis_distinct) {
			...
			fout_v_c_analysis << "\n  *  " << (*itr_v_c_analysis_distinct)->str_multinomial_catagory_name;
			for (size_type itr_s_genefamily_output = (*itr_v_c_analysis_distinct)->str_multinomial_catagory_name.length(); itr_s_genefamily_output < 20; ++itr_s_genefamily_output) {
				fout_v_c_analysis << " ";
			}
			...
			for (std::vector<fpf_data::peptide_data_type>::const_iterator itr_v_s_peptide_data = con_itr_v_s_peptide_data.begin(); itr_v_s_peptide_data != con_itr_v_s_peptide_data.end(); ++itr_v_s_peptide_data) {
				string_type con_s_genefamily = (*itr_v_c_analysis_distinct)->str_multinomial_catagory_name;
		...
		for (std::vector<fpf_data::multinomial_catagory_data_type*>::iterator itr_v_c_analysis = map_main_v_c_analysis_distict.begin(); itr_v_c_analysis != map_main_v_c_analysis_distict.end(); ++itr_v_c_analysis) {
			...
			if (((*itr_v_c_analysis)->v_s_peptide_data.size() != 0) && ((*itr_v_c_analysis)->d_score >= 0.01)) {
				fout_v_c_analysis << "\n\n\n\n\n" << (*itr_v_c_analysis)->str_multinomial_catagory_name;
				...
				for (std::vector<fpf_data::peptide_data_type>::const_iterator itr_v_s_peptide_data = con_itr_v_s_peptide_data.begin(); itr_v_s_peptide_data != con_itr_v_s_peptide_data.end(); ++itr_v_s_peptide_data) {
					...
					string_type con_s_genefamily = (*itr_v_c_analysis)->str_multinomial_catagory_name;
					size_type sw_s_peptideassociation_distinct = size_type();
					string_type con_s_genefamily_distinct = string_type();
					for (unsigned itr_s_genefamily = 0; itr_s_genefamily < con_s_genefamily.length(); ++itr_s_genefamily) {
						if (con_s_genefamily.at(itr_s_genefamily) == '*') {
